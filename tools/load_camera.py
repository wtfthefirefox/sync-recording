...
def load_camera(settings):
    ...
    url = f"{ip}/{api}/configureMonitor/{group}/{settings['mid']}/?data={data_string}"
...
if __name__ == '__main__':
    ...
    parser.add_argument("--ip", type=str, help="IP-адрес для доступа к Shinobi", default="0.0.0.0")
    parser.add_argument("--api", type=str, help="API для доступа к Shinobi", default="0")
    parser.add_argument("--group", type=str, help="Группа камер", default="1")
    parser.add_argument("--mid", type=str, help="ID камеры", default="10998")
    parser.add_argument("--name", type=str, help="Имя камеры", default="CameraName")
    parser.add_argument("--host", type=str, help="IP камеры", default="192.168.1.40")
    parser.add_argument("--port", type=str, help="Port камеры", default="554")
    parser.add_argument("--auto_host", type=str, help="RTSP-адрес камеры", default="rtsp://user:pass@192.168.1.40:554/")
    parser.add_argument("--detector", type=str, choices=["0", "1"], help="Включить/выключить детектор", default="1")
    parser.add_argument("--detector_trigger", type=str, choices=["0", "1"], help="Включение режима записи при детекции",
                        default="1")
    parser.add_argument("--detector_buffer_hls_time", type=str,
                        help="Количество секунд записи перед срабатыванием триггерра", default="5")
    parser.add_argument("--detector_send_frames", type=str, choices=["0", "1"], help="Включить/выключить анализ кадров",
                        default="0")
    parser.add_argument("--detector_timeout", type=str, help="Время записи с тригерра", default="1")
