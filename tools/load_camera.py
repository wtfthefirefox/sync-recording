...
async def load_camera(api, **kwargs):
    ...
    data_string = data_string[:-1] + ", 'details': " + f"'{d}'"
    data_string = data_string.replace("'", '"') + '}'

    url = ('http://51.250.48.249:8080/' + api +'/configureMonitor/1/') + data['mid'] + f"/?data=" + data_string
...
async def main():
    await load_camera(api='Y2xm1vUqY3dTR731r0OpN5pRXSJx9d', mid='test', name='load_test')
